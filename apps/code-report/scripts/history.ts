 ...
 * written to. `--no-hardlinks` keeps the source's object files untouched.
 */
...
export async function indexRefs(store: CodeGraphStore, options: IndexRefsOptions): Promise<IndexedRef[]> {
  ...
  for (const tag of options.refs) {
    ...
    const result = await indexPaths(store, { paths: rootsAt(options.repo), ref: tag.ref, commitHash: tag.commit });
