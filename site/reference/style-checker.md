...
| `runVultureAudit(files, { minConfidence })`, `VultureOptions` | vulture at `--min-confidence 60` by default; rule `vulture/<kind>` (`unused-function`, `unused-variable`, `unused-import`, `unused-class`, `unused-attribute`, `unused-method`, `unused-property`, `unreachable`) |
| `runPydoclintAudit(files, { style })`, `PydoclintOptions`, `DocstringStyle` | pydoclint `--quiet` in `numpy` style by default (`google`, `sphinx`); rule `pydoclint/DOCnnn`; a file pydoclint cannot parse (DOC002) is a `file-not-checked` failure |
| `runPyrightAudit(files, { pythonPath })`, `PyrightOptions`, `AUDIT_PYRIGHT_RULES`, `generatePyrightAuditConfig(root)` | pyright with a temporary config: `typeCheckingMode` off, only `reportUnnecessaryIsInstance`, `Comparison`, `Contains` and `Cast` on as warnings; rule `pyright/<rule>`, 1-based lines |
| `runImportLinter({ cwd })`, `NO_IMPORT_LINTER_CONFIG` | `lint-imports` only when `setup.cfg` or `.importlinter` has `[importlinter]` or `pyproject.toml` has `[tool.importlinter]`; one `error` per broken import chain, rule `import-linter/<contract-id>` (a slug of the name when pyproject gives no `id`), at the importing module's file |
| `countSuppressions(files, { cwd })`, `findSuppressions(file, text)`, `suppressionTotals(diagnostics)`, `NOQA_RULE`, `TYPE_IGNORE_RULE` | a text scan, no subprocess: one `suppression/noqa` or `suppression/type-ignore` warning per marker, and totals overall and per file |
| `AuditOptions`, `RunnerResult.warnings` | every Python audit runner returns an empty result whose warning names the tool and its `pip install` command when the binary is absent, and never throws for it |
